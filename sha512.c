...
char *sha512sum(const char *fileName) {
	SHA512_CTX ctx;
	
    unsigned char *result = malloc(SHA512_DIGEST_LENGTH);
    char *hexresult;
    ...
    while( (rbytes = read(fd, buf, BUFSIZE) ) > 0) {
        SHA512_Update(&ctx, buf, rbytes);
        memset(buf   , 0, BUFSIZE);
    }
    
	SHA512_Final(result, &ctx);
	
    hexresult = malloc(2*SHA512_DIGEST_LENGTH +1);
    memset(hexresult, 0, 2*SHA512_DIGEST_LENGTH+1);
